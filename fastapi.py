...
from proiect_classes import (
    ...
    IdentifierCharacteristicCreate, IdentifierCharacteristicResponse,
)
app = FastAPI()
...
def patch_identifier(identifier_name: str, identifier: IdentifierUpdate, db: Session = Depends(get_db)):
    ...
    if obj is None:
        raise HTTPException(status_code=404, detail="Identifier not found")
    for key, value in identifier.model_dump(exclude_none=True).items():  ##asta e ca sa nu facem aia cu is not none de 1000 de ori
        setattr(obj, key, value)
    db.commit()
    db.refresh(obj)
    return obj
...
# ─────────────────────────────────────────────
#  Relationships  (cheie compusă: from + to)
# ─────────────────────────────────────────────
...
@app.post("/IdentifierCharacteristics/", response_model=IdentifierCharacteristicResponse)
def create_identifier_characteristic(ic: IdentifierCharacteristicCreate, db: Session = Depends(get_db)):
    db_obj = IdentifierCharacteristic(**ic.model_dump())
    db.add(db_obj)
    db.commit()
    db.refresh(db_obj)
    return db_obj
...
@app.get("/IdentifierCharacteristics/", response_model=list[IdentifierCharacteristicResponse])
def read_all_identifier_characteristics(db: Session = Depends(get_db)):
    return db.query(IdentifierCharacteristic).all()
 
 
@app.get("/IdentifierCharacteristics/{identifier_name}", response_model=list[IdentifierCharacteristicResponse])
def read_identifier_characteristics(identifier_name: str, db: Session = Depends(get_db)):
    objs = db.query(IdentifierCharacteristic).filter(
        IdentifierCharacteristic.identifier_name == identifier_name
    ).all()
    if not objs:
        raise HTTPException(status_code=404, detail="No characteristics found for this identifier")
    return objs
 
 
@app.delete("/IdentifierCharacteristics/{identifier_name}/{master_name}/{characteristic_name}")
def delete_identifier_characteristic(identifier_name: str, master_name: str, characteristic_name: str, db: Session = Depends(get_db)):
    obj = db.query(IdentifierCharacteristic).filter(
        IdentifierCharacteristic.identifier_name == identifier_name,
        IdentifierCharacteristic.master_name == master_name,
        IdentifierCharacteristic.characteristic_name == characteristic_name
    ).first()
    if obj is None:
        raise HTTPException(status_code=404, detail="IdentifierCharacteristic not found")
    db.delete(obj)
    db.commit()
    return {"detail": f"IdentifierCharacteristic ({identifier_name}, {master_name}, {characteristic_name}) deleted"}

#la asta sunt toate chei primare deci nu merge sa ai update
